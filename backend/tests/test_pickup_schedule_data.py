...
def _load_items() -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute("SELECT data_json FROM pickup_schedule").fetchall()
    return [json.loads(row["data_json"]) for row in rows]


def test_pickup_schedule_has_no_duplicate_ids():
    items = _load_items()
    ids = [item["id"] for item in items]
    assert len(ids) == len(set(ids))


def test_pickup_schedule_covers_launch_through_2026():
    items = _load_items()
    years = {item["year"] for item in items}
    assert {2024, 2025, 2026}.issubset(years)


def test_pickup_schedule_has_every_month_from_launch():
    items = _load_items()
...
def test_pickup_schedule_entries_validate():
    items = _load_items()
    for item in items:
        PickupScheduleItem.model_validate(item)


def test_pickup_schedule_keeps_corrected_mid_2026_banners():
    items = _load_items()
...
def test_pickup_schedule_allows_rerun_3():
    items = _load_items()
