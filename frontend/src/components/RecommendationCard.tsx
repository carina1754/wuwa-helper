  ...
  return (
    <div className="rounded-lg border border-slate-300 bg-white/60 p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60">
      ...
      {damage ? (
        <div className="mt-3 rounded-md border border-indigo-200 bg-indigo-50/70 p-3 dark:border-indigo-900 dark:bg-indigo-950/40">
          <div className="flex items-baseline justify-between">
            <span className="text-sm font-medium text-slate-700 dark:text-neutral-200">파티 딜 지수</span>
          ...
          <p className="mt-1.5 text-[11px] text-slate-400 dark:text-neutral-500">
            ※ 표준 빌드(90레벨·추천무기·이상적 에코) 가정 기반 상대 지수 — 절대 수치가 아니라 조합 비교용이에요.
          </p>
        </div>
      ) : null}
            ...
            <div key={`${pick.resonator_id}-${i}`} className="rounded-md border border-slate-200 p-3 dark:border-neutral-800">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {pick.role ? (
                      <span className="rounded bg-slate-200 px-1.5 py-0.5 text-[11px] text-slate-700 dark:bg-neutral-800 dark:text-neutral-300">
                        {ROLE_LABEL[pick.role] ?? pick.role}
                      </span>
                    ) : null}
                  </div>
              ...
              {(pick.weapon || pick.echo) && (
                <div className="mt-3 flex flex-wrap gap-4">
                  {pick.weapon ? (
                    <div>
                      <p className="mb-1 text-[11px] font-medium text-slate-500 dark:text-neutral-400">무기</p>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-xs">
                          ...
                          {pick.weapon.alt_ids.length > 0 ? (
                            <div className="text-slate-400">
                              대안: {pick.weapon.alt_ids.map((a) => names.weapon.get(a) ?? a).join(", ")}
                            </div>
                          ) : null}
                        </div>
                      </div>
                  ...
                  {pick.echo ? (
                    <div>
                      <p className="mb-1 text-[11px] font-medium text-slate-500 dark:text-neutral-400">에코 / 소나타</p>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-xs">
                          {pick.echo.main_echo_id ? (
                            <div>메인: {names.echo.get(pick.echo.main_echo_id) ?? pick.echo.main_echo_id}</div>
                          ) : null}
                          ...
                          {pick.echo.sub_stats && pick.echo.sub_stats.length > 0 ? (
                            <div className="mt-0.5 text-slate-400">추천 추옵: {pick.echo.sub_stats.join(" · ")}</div>
                          ) : null}
                        </div>
                      </div>
                    </div>
      ...
      {rec.upgrade_order.length > 0 ? (
        <div className="mt-3">
          <p className="mb-1 text-sm font-medium">추천 업그레이드 순서</p>
