          ...
          {filteredCharacters.map((character) => (
            <article key={character.id} className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-panel">
              {character.splash_image ? (
                ...
                <img src={character.splash_image} alt="" className="h-40 w-full object-cover object-top" />
              ) : null}
              <div className="grid gap-3 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="truncate text-base font-semibold text-slate-950">{character.name}</h3>
                    <p className="mt-1 text-sm text-slate-500">
                      {character.rarity}★ · {character.element} · {character.weapon_type} · {t.roles[character.role]}
                    </p>
                  </div>
                </div>
